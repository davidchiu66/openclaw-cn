...
export const dingtalkConnectorOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  getStatus: async ({ cfg }) => {
    ...
    return {
      channel: CHANNEL_ID,
      configured,
      statusLines: [`钉钉连接器: ${configured ? "已配置" : "需要填写凭证"}`],
  ...
  configure: async ({ cfg, prompter }) => {
    ...
    // gatewayToken: default to existing gateway.auth.token if present
...
function applyDingtalkConnectorConfig(
  ...
  // Ensure gateway.http.endpoints.chatCompletions.enabled = true (required by the connector)
